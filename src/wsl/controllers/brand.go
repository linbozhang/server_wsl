...
type RawBrandController struct {
	beego.Controller
}

func (c *RawBrandController) URLMapping(){
	c.Mapping("Post",c.Post)
}
// Post ...
// @Title Post
// @Description create RawBrand
// @Param	body		body 	models.RawBrand	true		"body for RawBrand content"
// @Success 201 {int} models.Brand
// @Failure 403 body is empty
// @router / [post]
func (c *RawBrandController) Post() {
	f,h,err:=c.GetFile("uploadname")
	brandname:=c.GetString("brandname")
	if err!=nil{
		log.Fatal("rawbrand getfile err",err)
	}
	defer f.Close();
	path:="ar/public/brand/"+"brand_"+h.Filename
...
func (c *BrandController) GetAll() {
	...
	l, err := models.GetAllBrand(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
		c.Data["json"] = l
